...
//! hash map so sparse high IDs allocate only their own pages. A page is freed
//! as soon as every entry returns to the vacant value, so repeated
//! build/destroy churn settles at a stable footprint instead of tracking the
//! ID high-water mark.
//!
...
/// Sentinel stored for unmapped lane slots. Kept identical to the graph's
/// vacant marker so call sites keep their existing vacancy checks.
    ...
    T: Copy + PartialEq,
{
    ...
    /// Removal order does not affect the outcome, so hash-map iteration order
    /// cannot leak into simulation behavior.
    ...
    fn page_mut_or_insert(&mut self, page_id: u64) -> &mut [T; PAGE_SIZE] {
        if page_id < MAX_DIRECT_PAGES as u64 {
            ...
            return self.direct_pages[index]
                .get_or_insert_with(|| Box::new([vacant; PAGE_SIZE]))
                .as_mut();
        }
        let vacant = self.vacant;
        self.sparse_pages
            .entry(page_id)
            .or_insert_with(|| Box::new([vacant; PAGE_SIZE]))
            .as_mut()
    }
    ...
    fn storage_bytes(&self) -> usize {
        self.allocated_pages() * PAGE_SIZE * std::mem::size_of::<T>()
    }
}
    ...
    T: Copy + PartialEq + Hash,
{
    fn hash<H: Hasher>(&self, state: &mut H) {
        // Hash the logical contents in index order so equal maps hash
        // equally regardless of page layout or hash-map iteration order.
...
///
/// Pages are allocated for live transport entities only; removing an entity's
/// last lane frees its page when nothing else shares it.
...
mod tests {
    ...
    fn sparse_high_indices_allocate_only_their_pages() {
        ...
        assert_eq!(map.storage_bytes(), 2 * PAGE_SIZE * 4);
    }
    ...
    fn revision_retain_drops_rejected_indices() {
        ...
        map.set(EntityId::new(900_000), 9);

        map.inner.retain(|raw| raw != 261);

        assert_eq!(map.revision(EntityId::new(7)), 3);
        assert_eq!(map.revision(EntityId::new(261)), 0);
        assert_eq!(map.revision(EntityId::new(900_000)), 9);
        // The surviving low page stays allocated while the emptied high page
        // is gone entirely.
        assert_eq!(map.inner.allocated_pages(), 2);
    }
