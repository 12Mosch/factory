...
fn entity_layers(style: EntityVisualStyle) -> Vec<VisualLayer> {
    ...
    // Track is a path across its footprint rather than a building standing on
    // it, so it skips the shared shadow, relief, and body plate that would
    // otherwise draw a box around the rails.
    if style.kind.is_rail() {
        rail_layers(&mut builder, style);
        return builder.finish();
    }
    ...
    match style.kind {
        ...
        EntityKind::RailStraight | EntityKind::RailCurved | EntityKind::ResourcePatch => {}
    }
