...
use factory_sim::{
    AssemblerError, AssemblingMachineState, BELT_SUBTILES_PER_TILE,
    BURNER_MINING_DRILL_FUEL_SLOT_INDEX, BURNER_MINING_DRILL_OUTPUT_SLOT_INDEX, BurnerDrillError,
    CHUNK_SIZE, ContainerError, Direction, EntityFootprint, EntityId, FURNACE_FUEL_SLOT_INDEX,
...
#[derive(Component)]
struct AssemblerIngredientsText;

#[derive(Component)]
struct AssemblerProductsText;

#[derive(Component)]
struct AssemblerProgressText;
...
type AssemblerRecipeButtonInteractionQuery<'w, 's> = Query<
    ...
    (Changed<Interaction>, With<Button>),
>;
type AssemblerDetailTextQuery<'w, 's> = Query<
    'w,
    's,
    (
        &'static mut Text,
...
fn update_assembler_detail_text(
    sim: Res<SimResource>,
    open_container: Res<OpenContainer>,
...
fn update_assembler_recipe_button_colors(
    sim: Res<SimResource>,
    open_container: Res<OpenContainer>,
...
fn is_burner_drill_entity(sim: &Simulation, entity_id: EntityId) -> bool {
    open_machine_kind(sim, entity_id) == Some(OpenMachineKind::BurnerDrill)
}

fn is_furnace_entity(sim: &Simulation, entity_id: EntityId) -> bool {
    open_machine_kind(sim, entity_id) == Some(OpenMachineKind::Furnace)
}

fn is_assembler_entity(sim: &Simulation, entity_id: EntityId) -> bool {
...
fn open_machine_kind(sim: &Simulation, entity_id: EntityId) -> Option<OpenMachineKind> {
...
fn renderable_entity_style(sim: &Simulation, entity_id: EntityId) -> Option<(Color, Vec2)> {
...
fn format_item_display_name(catalog: &PrototypeCatalog, item_id: ItemId) -> String {
    catalog
        .items
        .get(item_id.index())
...
fn assembler_recipe_button_normal_color() -> Color {
    Color::srgba(0.16, 0.18, 0.18, 0.96)
}

fn assembler_recipe_button_selected_color() -> Color {
    Color::srgba(0.18, 0.43, 0.55, 0.98)
}

fn assembler_recipe_button_muted_color() -> Color {
