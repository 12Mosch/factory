...
pub struct SimulationTickProfile {
    ...
    /// Terrain creation and initialization for queued streaming requests.
...
impl TickProfiler for TickProfileCollector {
    ...
    fn finish(&mut self, phase: ProfilePhase, span: ProfileSpan) {
        ...
        match phase {
            ...
            ProfilePhase::Heat => self.profile.heat += elapsed,
            ProfilePhase::Circuits => self.profile.circuits += elapsed,
            ProfilePhase::Lamps => self.profile.lamps += elapsed,
            ProfilePhase::Power => self.profile.power += elapsed,
            ProfilePhase::Robots => self.profile.robots += elapsed,
            ProfilePhase::Radars => self.profile.radars += elapsed,
            ProfilePhase::Rails => self.profile.rails += elapsed,
