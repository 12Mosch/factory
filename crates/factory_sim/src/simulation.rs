...
impl<'a> TransportBeltAdvancement<'a> {
    fn new(entities: &'a mut EntityStore, tile_to_belt: BTreeMap<(i32, i32), EntityId>) -> Self {
        Self {
            entities,
            tile_to_belt,
...
fn transport_belt_tile_map(entities: &EntityStore) -> BTreeMap<(i32, i32), EntityId> {
    entities
        .transport_belts
        .keys()
...
impl EntityStore {
    ...
    fn entity_inventory(&self, entity_id: EntityId) -> Result<&Inventory, ContainerError> {
    ...
    fn lab_state(&self, entity_id: EntityId) -> Result<&LabState, LabError> {
        if !self.placed_entities.contains_key(&entity_id) {
    ...
    fn lab_state_mut(&mut self, entity_id: EntityId) -> Result<&mut LabState, LabError> {
        if !self.placed_entities.contains_key(&entity_id) {
            return Err(LabError::MissingEntity(entity_id));
        }
    ...
    fn furnace_state(&self, entity_id: EntityId) -> Result<&FurnaceState, FurnaceError> {
        if !self.placed_entities.contains_key(&entity_id) {
    ...
    fn belt_segment(&self, entity_id: EntityId) -> Result<&BeltSegment, BeltError> {
        if !self.placed_entities.contains_key(&entity_id) {
    ...
    fn belt_segment_mut(&mut self, entity_id: EntityId) -> Result<&mut BeltSegment, BeltError> {
        if !self.placed_entities.contains_key(&entity_id) {
            return Err(BeltError::MissingEntity(entity_id));
        }
    ...
    fn inserter_state(&self, entity_id: EntityId) -> Result<&InserterState, InserterError> {
        if !self.placed_entities.contains_key(&entity_id) {
    ...
    fn reserve_entity(&mut self, reservation: EntityReservation) -> EntityId {
    ...
    fn remove_placed_entity(&mut self, entity_id: EntityId) -> Option<PlacedEntity> {
