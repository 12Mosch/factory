...
| Rail signals | Green/yellow/red lamps | World-space `>` clear, `=` reserved, and `X` blocked symbols above both ordinary and chain signals; lamp luminance also orders reserved > clear > blocked |
...
reduce hue reliance where they exist (machines, threats, build status, rail
signals, wire thickness, selection width).
