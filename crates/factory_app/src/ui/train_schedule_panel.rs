...
mod tests {
    ...
    fn manual_mode_explains_how_to_drive_and_return_to_the_schedule() {
        ...
        let automatic_snapshot = ScheduleSnapshot {
            manual: false,
            rows: Vec::new(),
            has_stations: false,
        };
        let mut automatic_world = World::new();
        let mut automatic_queue = bevy::ecs::world::CommandQueue::default();
        {
        ...
        let mut automatic_texts = automatic_world.query::<&Text>();
        assert!(
            automatic_texts
                .iter(&automatic_world)
                .any(|text| text.0 == "Drive manually")
        );
    }
